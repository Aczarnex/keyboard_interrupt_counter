...
#include <linux/cdev.h>		// cdev interface, for registering character
				// devices
			     ...
			     unsigned long ioctl_carry)
{
	...
	case KIC_GET_COUNT_QUERY: {	// option for retrieving count
		long long kic_carry = atomic64_read(&kic_interrupt_count);
...
{
	atomic64_inc(&kic_interrupt_count);
	return IRQ_HANDLED;
}
...
static void kic_reset(void)
{
	...
	atomic64_set(&kic_interrupt_count, 0);
}
